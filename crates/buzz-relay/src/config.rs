...
pub struct CorporateIdentityConfig {
    ...
    ///
    /// This value is stored only in the private relay binding table and is
    /// never projected into a public Nostr event.
...
mod tests {
    ...
    fn removed_public_display_configuration_is_rejected() {
        let _guard = ENV_MUTEX.lock().unwrap();
        clear_removed_identity_provider_env();
        std::env::set_var("BUZZ_CORPORATE_IDENTITY_PUBLIC_DISPLAY_CLAIM", "email");

        let error = Config::from_env().expect_err("legacy public projection must be rejected");
        clear_removed_identity_provider_env();

        assert!(error
            .to_string()
            .contains("BUZZ_CORPORATE_IDENTITY_PUBLIC_DISPLAY_CLAIM was removed"));
        assert!(error.to_string().contains(CONFIG_ENV));
    }
