...
pub async fn cmd_history(
    ...
    // query returns a revision history that is silently empty.
    let resp = client
        .query_multi(&[
            json!({"kinds": COMMENT_STREAM_KINDS, "#e": roots, "limit": limit}),
            json!({"kinds": [KIND_GIT_PR_UPDATE], "#E": roots, "limit": limit}),
        ])
        .await?;
    println!("{resp}");
    Ok(())
...
pub async fn dispatch(command: ProjectsCmd, client: &BuzzClient) -> Result<(), CliError> {
    match command {
        ...
        ProjectsCmd::History { roots, limit } => cmd_history(client, &roots, limit).await,
    }
}
