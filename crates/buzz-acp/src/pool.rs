...
pub async fn run_prompt_task(
    ...
    // ACP process.
    let project_origin = batch.as_ref().and_then(FlushBatch::project_origin);
    let resolved_channel_info = match (&source, project_origin) {
        // Project-routed batches use a UUIDv5 route key, not a real channel id.
        // Resolving it as channel metadata would fail the new authoritative
        // project-home check before the deliberately separate project context
        // fetch gets a chance to run.
        (PromptSource::Channel(_), Some(_)) => None,
        (PromptSource::Channel(channel_id), None) => {
            match ctx.channel_info.resolve(*channel_id).await {
                Ok(info) => info,
                Err(error) => {
                    tracing::warn!(
                        channel_id = %channel_id,
    ...
    let prompt_sections: Vec<String> = if let Some(text) = prompt_text {
        ...
        // Reuse the authoritative channel/project-home result resolved before
        // any ACP session boundary. Project-routed batches intentionally have
        // no channel metadata: their UUIDv5 route key names a root, not a
        // channel.
