...
import {
  ...
  transcriptByAgent,
  snapshotByAgent,
  evictionFloorByAgent,
  archiveEventsByChannel,
  latestLiveSessionByAgentChannel,
  liveSessionKey,
  controlResultListeners,
  agentManagementListeners,
  projectChannelRequestListeners,
  knownAgentPubkeys,
  knownAgentsBySubscription,
  pendingUnknownAgentFrames,
  type ObserverSnapshot,
  type AgentObserverStoreListener,
  type AgentObserverStoreUpdate,
} from "./observerRelayStoreState";
export { getLatestLiveSessionId } from "./observerRelayStoreState";
export type {
  ObserverSnapshot,
  AgentObserverStoreUpdate,
} from "./observerRelayStoreState";

/**
 * Observer frames the ingest gate refused, keyed by normalized agent pubkey. A
 * frame reaching this app is already `#p`-addressed to our own identity, so a
 * refusal here means the relay routed us telemetry for an agent we cannot
