...
14. **Shared instructions must be reviewable byte-for-byte.** Agent definitions
...
15. **Profile runtime sections render only reported agent data.** Missing
   ...
   content.
16. **Thinking effort has two surfaces: a local-only WRITE control and a
...
17. **Owner-only builds discover only verified same-owner remote agents.**
