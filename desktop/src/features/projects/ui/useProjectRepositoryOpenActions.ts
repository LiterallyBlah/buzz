...
export function useProjectRepositoryOpenActions({
  ...
  const handleOpenMergeRecoveryTerminal = React.useCallback(
    async (input: MergeRecoveryInput) => {
      const targetCloneUrl = repository?.cloneUrls[0] ?? input.targetCloneUrl;
      if (!repository || !targetCloneUrl) {
        throw new Error(
          repository
            ? "This project has no clone URL. Add one to the repository announcement, or reconnect to the relay that hosts it."
            : "No project selected.",
        );
      }
