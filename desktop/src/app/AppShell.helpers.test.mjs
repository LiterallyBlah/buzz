...
test("project notification activation routes to the forced project view", async () => {
  const calls = [];
  await activateDesktopNotificationTarget(
    {
      channelId: null,
    ...
    },
    {
      goChannel: async () => calls.push("channel"),
      goHome: async () => calls.push("home"),
      goProject: async (projectId, behavior) =>
        calls.push(`${projectId}:${String(behavior?.force)}`),
      openSearchHit: async () => calls.push("message"),
      revealWindow: async () => calls.push("reveal"),
    },
  );

  assert.deepEqual(calls, ["project-1:true", "reveal"]);
});
