...
//!
//! Split out of `app_state.rs` under the same `#[path]` convention as
//! `app_state_keyring.rs`: these are lock-wrapping helpers over fields
//! declared there, not state of their own, and the struct file is at the
