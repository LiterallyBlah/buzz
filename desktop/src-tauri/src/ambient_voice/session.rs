...
pub(crate) struct Transcript {
    ...
    /// [`AmbientSessionConfig::mute_epochs`] as of the moment the capture was
    /// armed — the capture this text belongs to died if it moved.
...
/// epoch, and the pill.
///
/// One function rather than lines in [`AmbientSession::set_muted`] so the
/// mute-fence regressions drive the exact writes the command path performs —
