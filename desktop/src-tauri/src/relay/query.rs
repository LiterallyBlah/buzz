...
/// Issue an authenticated `POST /query` and parse the response under the
/// supplied whole-request deadline.
///
/// Kept as the shared ordinary-query seam so the live release's stalled-header,
/// stalled-success-body, and stalled-error-body regressions continue to drive
/// the exact production request path after the M1 module split.
pub(crate) async fn send_query_request(
