...
// ── Wake-word (KWS) model ─────────────────────────────────────────────────────
//
// Downloaded only when the `ambientVoice` preview feature is switched on, not
// at every launch: it is ~18 MB of preview-only capability and most users will
...
impl ModelManager {
    ...
    pub fn new() -> Option<Self> {
        ...
        let manager = Self {
            ...
            tts: tts_model_slot(),
            kws: ModelSlot::new(KWS_MODEL_DIR_NAME, KWS_EXPECTED_FILES, KWS_MODEL_VERSION),
        };
